import { AlertTriangle, Landmark, Eye } from "lucide-react";
...
export function BankAccountCard({
  ...
  const movementLimit = account.visibleMovementLimit;
  const movementUsed = account.visibleMovementsUsed;
  const hasMovementLimit = movementLimit !== undefined && movementUsed !== undefined;
  const remainingMovements = hasMovementLimit
    ? Math.max(0, movementLimit - movementUsed)
    : null;
  const movementRatio = hasMovementLimit && movementLimit > 0
    ? movementUsed / movementLimit
    : 0;
  const showMovementWarning =
    hasMovementLimit &&
    movementRatio >= (account.movementWarningThreshold ?? 0.8);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {hasMovementLimit && showMovementWarning && (
        <div className="mt-4 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2.5">
          <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />
          <div>
            <p className="text-xs font-semibold text-amber-900">
              Límite de movimientos cercano
            </p>
            <p className="text-[11px] text-amber-800">
              {remainingMovements === 0
                ? "Alcanzaste el límite de movimientos visibles."
                : `${remainingMovements} movimientos visibles restantes.`}
            </p>
          </div>
        </div>
      )}
