...
function computeTotals(
  ...
  const totalCashReserved = cash.reservedOperations.reduce(
    (sum, op) => sum + op.amount,
    0
  );

  const totalBankReal = banks.reduce(
...
export function BalanceSummary({
  ...
  const { totalControlled, totalReserved, totalAvailable } =
    computeTotals(cash, banks);

  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {/* Hero: Dinero total controlado */}
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Efectivo y saldos bancarios registrados
        </p>
