...
interface AddParticipantModalProps {
  isOpen: boolean;
  onClose: () => void;
  availableUsers: Omit<ShiftParticipant, "id" | "joinedAt" | "leftAt" | "status" | "shiftRole">[];
  onAdd: (user: Omit<ShiftParticipant, "id" | "joinedAt" | "leftAt" | "status" | "shiftRole">) => void;
}
...
export function AddParticipantModal({ isOpen, onClose, availableUsers, onAdd }: AddParticipantModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[90dvh] w-full max-w-lg flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 p-6">
          <h2 className="text-lg font-semibold text-slate-900">Agregar participante</h2>
          <button
            ref={closeButtonRef}
            onClick={onClose}
          ...
          <div className="mt-4 space-y-2">
            {availableUsers.map((user) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">{systemRoleLabels[user.systemRole]}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-200 p-6">
          ...
          <Button variant="default" onClick={handleConfirm} disabled={!selectedUser}>
            Agregar
          </Button>
        </div>
      </div>
