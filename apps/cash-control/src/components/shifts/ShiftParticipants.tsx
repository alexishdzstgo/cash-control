  ...
  return (
    <div className="rounded-xl border border-brand-border bg-white p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {activeParticipants.length} activo{activeParticipants.length !== 1 ? "s" : ""}
            {leftParticipants.length > 0 && ` · ${leftParticipants.length} retirado${leftParticipants.length !== 1 ? "s" : ""}`}
          </p>
        </div>
        ...
        {activeParticipants.length === 0 ? (
          <div className="mt-4 rounded-lg border border-dashed border-slate-300 p-8 text-center">
            <p className="text-sm text-slate-500">No hay participantes activos en este turno.</p>
          </div>
        ) : (
      ...
      {leftParticipants.length > 0 && (
        <div className="mt-6">
          <h4 className="mb-3 text-sm font-medium text-slate-500">Participantes retirados</h4>
