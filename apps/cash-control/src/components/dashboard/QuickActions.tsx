    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {!currentShift && (
        <p className="mb-3 text-sm text-slate-500">
          Inicia un turno para registrar operaciones.
        </p>
      )}
      {!userHasActiveParticipation && (
        <p className="mb-3 text-sm text-slate-500">
          Activa tu participación para registrar operaciones.
        </p>
      )}
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {quickActions.map((action) => {
          ...
          const disabled =
            action.href !== "/history" &&
            (!userHasActiveParticipation || currentShift?.status !== "open");
          const content = (
            <>
          ...
          return disabled ? (
            <div
              key={action.href}
              aria-disabled="true"
              className="flex cursor-not-allowed items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3.5 opacity-60"
            >
              {content}
            </div>
          ) : (
